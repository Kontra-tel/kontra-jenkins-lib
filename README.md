...
- Tokens in the last commit message: `!major`, `!minor` (default is patch). `!release` sets `isRelease`.
...
Creates an annotated tag and pushes it. Optionally creates/updates a GitHub Release.

- Tagging gates:
	- `alwaysTag: true` → always tag
	- `tagOnRelease: true` (default) + last commit contains `!release` OR `forceRelease: true`
...
- GitHub Release (any of):
	- `createGithubRelease: true`, or commit contains `!ghrelease`, or `forceGithubRelease: true` (also honors `!no-ghrelease`).
...
pipeline {
	...
	stages {
		...
		stage('Release') {
			when { expression { env.IS_RELEASE == 'true' } }
			steps { script { release(version: env.BUILD_VERSION, credentialsId: 'github-token', createGithubRelease: true) } }
		}
